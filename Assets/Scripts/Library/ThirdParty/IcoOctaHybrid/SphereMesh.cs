// courtesy of Sebastian Lague https://github.com/SebLague/Solar-System/blob/Episode_02/Assets/Celestial%20Body/Scripts/SphereMesh.cs (modified)
...
public class SphereMesh {
	...
	public SphereMesh (int resolution, float scale) {
	...
	{
		Vector3 translate = Vector3.zero - position;
	...
	public Mesh GetMesh(float noiseScale, float dispInt)
	{
		Mesh mesh = new Mesh();
		...
		InitDisplace(mesh, noiseScale, dispInt);
	...
	private void InitDisplace(Mesh mesh, float scale, float dispInt)
	{
		...
		{
			vertices[i] += normals[i] * util.PerlinNoise(vertices[i].x, vertices[i].y, scale) * dispInt;
		}
